...
#image_dim_ordering() is used for identifying it is using tensorflow
#nn.get_img_output_length directly sending function,it will be used in that file
...
''' 
reduce_lr = ReduceLROnPlateau(monitor='val_loss', factor=0.1, patience=5, verbose=0, mode='auto', min_lr=1e-9)
optimizer = Adam(lr=1e-5)
optimizer_classifier = Adam(lr=1e-5)
model_rpn.compile(optimizer=optimizer, loss=[losses.rpn_loss_cls(num_anchors), losses.rpn_loss_regr(num_anchors)])
model_classifier.compile(optimizer=optimizer_classifier, loss=[losses.class_loss_cls, losses.class_loss_regr(len(classes_count)-1)], metrics={'dense_class_{}'.format(len(classes_count)): 'accuracy'})
model_all.compile(optimizer=sgd, loss='mae',callbacks = [reduce_lr],metrics=['accuracy'])

epoch_length_train = len(train_imgs)
epoch_length_test = len(val_imgs)
...
for epoch_num in range(num_epochs):
	...
	progbar = generic_utils.Progbar(epoch_length_train)
	...
	while True:
		try:
			...
			R = roi_helpers.rpn_to_roi(P_rpn[0], P_rpn[1], C, K.image_dim_ordering(), use_regr=True, overlap_thresh=0.50, max_boxes=300)
...
############################# FOR TEST DATA ############################
	...
	progbar = generic_utils.Progbar(epoch_length_test)
	print('Epoch {}/{}'.format(epoch_num + 1, num_epochs))
	...
	while True:
		try:
			...
			if iter_num == epoch_length_test:
				...
				if C.verbose:
					print('Total loss is {}, testings done'.format(curr_loss))
