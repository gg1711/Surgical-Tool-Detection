...
dataset_val=np.zeros((epoch_length,7))
dataset_pred=np.zeros((epoch_length,7))
...
while True :
	try:
		...
		roi_pos.append(len(pos_samples))
		roi_neg.append(len(neg_samples))
		...
		rcnn_pred = model_classifier.predict_on_batch([X, X2[:, sel_samples, :]])

		############# calc of no. of objects #############
		...
		for i in range(real[0].shape[0]):
			for j in range(7):
				if (real[0][i][j]==1):
					dataset_val[iter_num][j]=1
	
		for i in range(obtain.shape[0]):
			for j in range(7):
				if (real[0][i][j]>0.0001):
					dataset_pred[iter_num][j]=1
...
for i in y1[0]:
	print(i)
print(dataset_calc.shape)
for i in dataset_calc:
	print(i)
print(Y1[0].shape)
for i in Y1[0]:
	print(i)
...
for i in dataset_pred:
	print(i)
np.save('valt.npy',dataset_val)
np.save('predt.npy',dataset_pred)
'''
print(dataset_pred.shape)
for i in dataset_pred:
	print(i)
print(img_data['bboxes'])
print(type(img_data['bboxes'][0]['class']))
print(class_mapping)
