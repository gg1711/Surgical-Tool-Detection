...
parser.add_option("-p", "--path", dest="test_path", help="give the video name.")
parser.add_option("-n", "--num_rois", type="int", dest="num_rois",
				help="Number of ROIs per iteration. Higher means more memory use.", default=15)
...
while(ret):
	...
	for i in range(5):
		...
		R = roi_helpers.rpn_to_roi(Y1, Y2, C, K.image_dim_ordering(), overlap_thresh=0.35)
		...
		for i in bboxes.keys():
			frame[0,class_map[i]]=1
		frame5.append(frame)
	frame5=np.vstack(frame5)
	frame=np.sum(frame5,axis=0)
...
np.save('all_frames_avg.npy',all_frames)
